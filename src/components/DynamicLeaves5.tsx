...
export const DynamicLeaves: React.FC<DynamicLeavesProps> = ({
  ...
  console.log("🍂 DynamicLeaves5 component mounted!");
  const meshRef = useRef<THREE.InstancedMesh>(null);
  const [meshReady, setMeshReady] = useState(false);
  const { camera } = useThree();

  // Load leaf texture
  const leafTexture = useLoader(
    THREE.TextureLoader,
  ...
  // GPU-accelerated shader material with leaf texture
  const material = useMemo(() => {
    return new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          vec4 leafColor = texture2D(leafTexture, vUv);
          
          // Use the texture's alpha channel for transparency
          // Tint the texture with the random color variation
          vec3 finalColor = leafColor.rgb * vColor;
          
          gl_FragColor = vec4(finalColor, leafColor.a * 0.9);
        }
      `,
  ...
  useEffect(() => {
    console.log("🍂 DynamicLeaves5 initializing...", { count, areaSize });
  ...
  console.log("🍂 Rendering", count, "leaves with texture");
