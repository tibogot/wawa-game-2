...
void main() {
  ...
  playerLeanAngle = mix(0.0, playerInteractionParams.y, playerFalloff);
  ...
  SKY_RATIO = 0.85;
  UP = vec3(0.0, 1.0, 0.0);
  skyFadeIn = highLODOut * SKY_RATIO;
  normal1 = normalize(mix(grassVertexNormal1, UP, skyFadeIn));
  normal2 = normalize(mix(grassVertexNormal2, UP, skyFadeIn));
...
export default function ClaudeGrassQuick5({
  ...
  const _projScreenMatrix = useRef(new THREE.Matrix4());
  const _baseCellPos = useRef(new THREE.Vector3());
  const _cameraPosXZ = useRef(new THREE.Vector3());
  const _aabbTmp = useRef(new THREE.Box3());
  const _currentCell = useRef(new THREE.Vector3());
  const _aabbSize = useRef(new THREE.Vector3());
  ...
  // Shared uniform update helper — avoids duplicating ~90 lines for low/high materials
  const updateMaterialUniforms = (mat) => {
    const u = mat.userData.shader;
    if (!u) return;
    const uf = u.uniforms;
    uf.time.value = totalTime.current;
    uf.playerPos.value.copy(playerPosRef.current);
    uf.viewMatrixInverse.value = camera.matrixWorld;
    uf.grassSize.value.set(grassWidth, grassHeight);
    uf.grassParams.value.z = heightScale;
    uf.grassParams.value.w = heightOffset;
    uf.grassDraw.value.set(lodDistance, maxDistance, 0, 0);
    uf.heightParams.value.x = terrainSize;
    uf.windParams.value.set(windDirectionScale, windDirectionSpeed, windStrengthScale, windStrengthSpeed);
    uf.windStrength.value = windEnabled ? windStrength : 0.0;
    uf.playerInteractionParams.value.set(
      playerInteractionEnabled ? playerInteractionRange : 999.0,
      playerInteractionEnabled ? playerInteractionStrength : 0.0,
    );
    uf.playerInteractionRepel.value = playerInteractionRepel ? 1.0 : -1.0;
    uf.playerInteractionHeightThreshold.value = playerInteractionHeightThreshold;
    uf.uBaseColor1.value.copy(baseColor1Ref.current);
    uf.uBaseColor2.value.copy(baseColor2Ref.current);
    uf.uTipColor1.value.copy(tipColor1Ref.current);
    uf.uTipColor2.value.copy(tipColor2Ref.current);
    uf.uGradientCurve.value = gradientCurve;
    uf.uAoEnabled.value = aoEnabled;
    uf.uAoIntensity.value = aoIntensity;
    uf.uFogEnabled.value = fogEnabled;
    uf.uFogNear.value = fogNear;
    uf.uFogFar.value = fogFar;
    uf.uFogIntensity.value = fogIntensity;
    uf.uFogColor.value.copy(fogColorRef.current);
    uf.uSpecularEnabled.value = specularEnabled;
    uf.uSpecularIntensity.value = specularIntensity;
    uf.uSpecularColor.value.copy(specularColorRef.current);
    uf.uSpecularDirection.value.set(specularDirectionX, specularDirectionY, specularDirectionZ).normalize();
    uf.uGrassMiddleBrightnessMin.value = grassMiddleBrightnessMin;
    uf.uGrassMiddleBrightnessMax.value = grassMiddleBrightnessMax;
    uf.uBackscatterEnabled.value = backscatterEnabled;
    uf.uBackscatterIntensity.value = backscatterIntensity;
    uf.uBackscatterColor.value.copy(backscatterColorRef.current);
    uf.uBackscatterPower.value = backscatterPower;
  ...
  const lowPoolIdx = useRef(0);
  const highPoolIdx = useRef(0);
  ...
  useFrame((state, delta) => {
    ...
    // Frustum culling setup — reuse refs, zero allocations
    const frustum = _frustum.current;
    const projScreenMatrix = _projScreenMatrix.current;
    ...
    // Calculate base cell position using patchSpacing — reuse ref
    const baseCellPos = _baseCellPos.current;
    baseCellPos.copy(camera.position);
    ...
    // Reset pool indices instead of spreading arrays
    lowPoolIdx.current = 0;
    highPoolIdx.current = 0;

    const cameraPosXZ = _cameraPosXZ.current;
    cameraPosXZ.set(camera.position.x, 0, camera.position.z);

    const aabbTmp = _aabbTmp.current;
    const currentCell = _currentCell.current;
    const aabbSize = _aabbSize.current;
    aabbSize.set(patchSpacing, 1000, patchSpacing);
    ...
    for (let x = -gridSize; x < gridSize; x++) {
      for (let z = -gridSize; z < gridSize; z++) {
        ...
        aabbTmp.setFromCenterAndSize(currentCell, aabbSize);
        ...
        const pool = useLowLOD ? meshPoolLow.current : meshPoolHigh.current;
        const poolIdx = useLowLOD ? lowPoolIdx : highPoolIdx;
        const geo = useLowLOD ? geometryLow : geometryHigh;
        const mat = useLowLOD ? materialLow : materialHigh;

        let mesh;
        if (poolIdx.current < pool.length) {
          mesh = pool[poolIdx.current];
        } else {
