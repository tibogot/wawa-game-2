...
void main() {
  ...
  randomAngle = hashVal1.x * 2.0 * PI; // Restore rotation variation - blades face different directions
  randomShade = 1.0; // No color variation - all blades same brightness
  randomHeight = 1.0; // Fixed height - all blades same height (no variation)
  randomWidth = 1.0; // Fixed width - all blades same width
  randomLean = 0.15; // Fixed lean - all blades same curvature/shape

  hashGrassColour = hash22(vec2(grassBladeWorldPos.x, grassBladeWorldPos.z));
  leanAnimation = 0.0; // Disabled - no animation variation to keep all blades consistent
  ...
  // Use fixed width for consistency - all blades same width
  grassTotalWidth = grassSize.x * mix(grassTotalWidthHigh, grassTotalWidthLow, highLODOut);

  // Shift verts
  x = (xSide - 0.5) * grassTotalWidth;
  y = heightPercent * grassTotalHeight;
  
  // Calculate smooth position across blade width (0.0 = left edge, 1.0 = right edge) for better normal blending
  // This will be interpolated in fragment shader between vertices
  xSideSmooth = xSide;

  // Wind - disabled for consistency (all blades same shape)
  windDir = 0.0;
  windNoiseSample = 0.0;
  windLeanAngle = 0.0;
  windAxis = vec3(1.0, 0.0, 0.0);
  windLeanAngle = 0.0;

  // Player interaction - disabled for consistency (all blades same shape)
  distToPlayer = 0.0;
  heightDiff = 0.0;
  heightFalloff = 0.0;
  distanceFalloff = 0.0;
  playerFalloff = 0.0;
  playerLeanAngle = 0.0;
  grassToPlayer = vec3(0.0, 0.0, 0.0);
  playerLeanAxis = vec3(1.0, 0.0, 0.0);

  randomLean += leanAnimation;

  // Fixed easedHeight - no LOD variation in curve calculation
  // Ensure bottom is flat: when heightPercent = 0, curveAmount should be 0
  easedHeight = easeIn(heightPercent, 2.0);
  curveAmount = -randomLean * easedHeight;
  // Clamp to ensure bottom vertices (heightPercent = 0) have no curve
  curveAmount = mix(0.0, curveAmount, step(0.01, heightPercent));

  // Normal calculation - COMPLETELY FIXED for all blades
  // Use a fixed normal direction that doesn't depend on rotation or curve
  // This ensures ALL blades have IDENTICAL lighting response = same color
  
  // Fixed normal: points straight up with slight forward tilt for rounded appearance
  vec3 fixedNormalBase = normalize(vec3(0.0, 1.0, 0.1));
  
  // Create two edge normals for rounded appearance - FIXED, same for all blades
  // These will blend across blade width for rounded effect
  vec3 fixedNormal1 = rotateY(PI * 0.3) * fixedNormalBase;
  vec3 fixedNormal2 = rotateY(-PI * 0.3) * fixedNormalBase;

  // Build grass matrix for position/rotation (but DON'T transform normals by it)
  ...
  // DON'T transform normals by grassMat - keep them in world space, fixed direction
  // This ensures all blades have the SAME normal direction = SAME lighting = SAME color
  grassVertexNormal1 = fixedNormal1; // Fixed edge normal 1, same for all blades
  grassVertexNormal2 = fixedNormal2; // Fixed edge normal 2, same for all blades
  
  // Keep old variables for compatibility
  float fixedCurve = -randomLean * easedHeight;
  fixedCurve = mix(0.0, fixedCurve, step(0.01, heightPercent));
  ncurve1 = fixedCurve;
  ncurve2 = fixedCurve * 0.9;
  n1 = vec3(0.0, heightPercent, 0.0);
  n2 = vec3(0.0, heightPercent * 0.9, 0.0);
  ncurve = normalize(n1 - n2);
  grassVertexNormal = fixedNormalBase;
  ...
  // FOR DEBUGGING: Use only ONE base color and ONE tip color (no variation)
  b1 = uBaseColor1;
  b2 = uBaseColor1; // Use same as b1 - no variation
  t1 = uTipColor1;
  t2 = uTipColor1; // Use same as t1 - no variation

  // No color variation - all blades use same base and tip colors
  // Disable LOD color mixing - use same color calculation for all blades regardless of distance
  baseColour = b1; // Use first base color for all blades
  tipColour = t1; // Use first tip color for all blades
  // Use consistent color calculation for all blades (no LOD variation)
  vGrassColour = mix(baseColour, tipColour, easeIn(heightPercent, uGradientCurve)) * randomShade;
  vGrassParams = vec4(heightPercent, grassBladeWorldPos.y, highLODOut, xSideSmooth);

  SKY_RATIO = 0.25;
  UP = vec3(0.0, 1.0, 0.0);
  skyFadeIn = 1.0; // Fixed - no distance-based normal variation
  // Use consistent normals for all blades - no mixing with UP vector
  normal1 = normalize(grassVertexNormal1);
  normal2 = normalize(grassVertexNormal2);
  ...
  // Disable view-dependent thickening for consistency - all blades same appearance from all angles
  viewDotNormal = 0.5; // Fixed value - no view-dependent variation
  viewSpaceThickenFactor = 0.0; // Disabled - no view-space thickening
...
void main() {
  ...
  // Fixed grassMiddle - use constant value for debugging (no brightness variation)
  grassMiddle = 1.0; // Fixed to max brightness - no variation
  ...
  // Always apply rounded normal blending across blade width for consistent appearance
  // vGrassParams.w is xSideSmooth (0.0 = left edge, 1.0 = right edge), interpolated in fragment shader
  // Use smooth interpolation for better rounded normal effect
  float blendFactor = saturate(vGrassParams.w);
  normal = normalize(mix(grassBaseNormal, normal2, blendFactor));
...
// Main component
export default function ClaudeGrassQuick4({
  ...
  useFrame((state, delta) => {
    ...
    if (Math.floor(totalTime.current * 60) % 60 === 0) {
      console.log("🌿 ClaudeGrassQuick4 playerPosition prop:", playerPosition);
      console.log("🌿 ClaudeGrassQuick4 playerPosRef:", playerPosRef.current);
    }
